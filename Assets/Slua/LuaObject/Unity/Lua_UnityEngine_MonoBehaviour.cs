...
public class Lua_UnityEngine_MonoBehaviour : LuaObject {
	...
	[SLua.MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]
	[UnityEngine.Scripting.Preserve]
	static public int get_destroyCancellationToken(IntPtr l) {
		try {
			#if DEBUG
			var method = System.Reflection.MethodBase.GetCurrentMethod();
			string methodName = GetMethodName(method);
			...
			UnityEngine.MonoBehaviour self=(UnityEngine.MonoBehaviour)checkSelf(l);
			pushValue(l,true);
			pushValue(l,self.destroyCancellationToken);
			return 2;
		}
		catch(Exception e) {
			return error(l,e);
	...
	static public void reg(IntPtr l) {
		...
		addMember(l,print_s);
		addMember(l,"destroyCancellationToken",get_destroyCancellationToken,null,true);
